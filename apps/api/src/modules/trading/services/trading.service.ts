...
export class TradingService {
  ...
  async adminSettlePendingOrder(orderId: string, actorId: string) {
    ...
    try {
      ...
      await this.validateBalances(order.userId, order.side, quantityGram, netRial);
      ...
      const journal = await this.postTradeJournal({
        ...
        side: order.side,
        quantityGram,
        grossRial: Number(order.grossRial),
        commissionRial: Number(order.commissionRial),
        netRial,
        idempotencyKey: order.idempotencyKey,
        orderId: order.id,
        description: `Admin settlement ${order.orderNumber}`,
      });
      ...
      await this.tradingRepository.createAuditLog(
        ...
        actorId,
        { reason: message, via: 'admin' },
      );
      throw error;
    }
  }

  async adminCancelPendingOrder(orderId: string, actorId: string, reason: string) {
    const order = await this.tradingRepository.findById(orderId);
    if (!order) {
      throw new NotFoundException('Trade order not found');
    }
    ...
    await this.tradingRepository.markFailed(order.id, reason);
    await this.tradingRepository.createAuditLog(
      order.id,
