...
/** ساعت شمسی (منطقه زمانی تهران) */
export function formatPersianTime(value: Date | string): string {
  const date = parseDateInput(value);
  if (!date) {
    return '—';
  }
  const { hour, minute } = toJalaali(date);
  return `${formatPlainFa(hour, { minimumIntegerDigits: 2 })}:${formatPlainFa(minute, { minimumIntegerDigits: 2 })}`;
}
